    ...
    try {
      ...
      if (fromLazadaScraper !== null) {
        ...
        const response = await fetch(
          "https://pc-x5qm.onrender.com/input/component_compatibility_checker",
          {
            method: "POST",
            headers: {
...
function generateDataContainerHTML(data) {
  ...
  if (item_count == 2) {
    ...
    return (
      '<div class="early_message"><strong>item_count Error: ' +
      item_count +
      " </strong> Please put <strong> two</strong>  item only in cart.</div>"
    );
}
